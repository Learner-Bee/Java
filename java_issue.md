...
##15、jdbc操作数据库时，如何提升读取数据和更新数据的性能
...
* 通常定义基本数据类型的变量、对象的引用变量、函数调用的现场保存都使用JVM的栈空间
...
str放在栈中；new String放在堆中；“hello”字面量放在方法区
...
###36.1 grep 强大文本搜索工具（Global Regular Expression Print）。可对文件内容、文件、进程进行查找操作
...
* 整行中匹配内容  
ll|awk '/Feb/{print $1}'  匹配Feb的行，并且打印第1列  
...
例：  
sed '1a drink ' ab 在文件ab的第一行后面增加一行字符串drink  
...
####1、批量替换字符串（先匹配到行，再替换行中的匹配字符串） 
替换一行中的某个部分格式：sed 's/要替换的字符串/新的字符串/g' (要替换的字符串可以用正则表达式)  
sed -i 's/test/go/g' file 将文件file中匹配的test字符串替换为go
